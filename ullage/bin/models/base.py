...
from bin.config.config import RAW_DATABASE_CONNECTION, PRODUCTION_DATABASE_CONNECTION
...
def reset_seconds_in_df_indexes(df):
    ...
    print("reset_seconds_in_df_indexes : ",df)
    print()
...
class Common():
    ...
    def get_engine(cls):
        ...
        if cls.__module__ == 'bin.models.production':
            return production_engine
        elif cls.__module__ == 'bin.models.raw':
            return raw_engine
    ...
    def write_df(cls, df, only_insert=True, index=True):
        ...
        print("dataframe load to tables in write_df classmethod : " ,df)
        print()
