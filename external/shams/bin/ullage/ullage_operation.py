...
from models import production as production_models
    ...
    def write_raw_measures_and_set_alarms(self, selected_date):
        ...
        with Session(self.production_models.engine) as session:
            for sensor in session.scalars(select(self.production_models.UllageSensor)):
                ...
                #     if alarm_definition.definition_valid_to is None:  # We try only the activated alarm definitions
                ...
                #             sensor_raw_df[alarm_definition.name] = (sensor_raw_df['value'] < alarm_definition.value)
                #
                # self.production_models.UllageAlarmDefinition.write_df(
                #     sensor_raw_df,
                #     only_insert= True
                # )
        ...
        #
        #         for alarm_definition in sensor.alarm_definitions:
        #             for k, v in sensor_raw_df.groupby((sensor_raw_df[alarm_definition.name].shift() != sensor_raw_df[
        #                 alarm_definition.name]).cumsum()):
        #                 if v[alarm_definition.name].iloc[0]:
        ...
        #
        #             self.production_models.PumpAlarmRecord.write_df(
        #                 sensor_alarm,
        #                 only_insert=True
        #             )
